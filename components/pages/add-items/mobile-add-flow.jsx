...
export function MobileAddFlow({ onClose, initialView = 'SCAN_VIEW' }) {
  ...
  const [expirationPrecision, setExpirationPrecision] = useState('day');
    ...
    const lookup = async () => {
      ...
      try {
        ...
        if (data.found && data.data) {
          ...
          if (data.data.unit) setUnit(data.data.unit);
          if (data.data.weightPerUnit) setWeightPerUnit(String(data.data.weightPerUnit));
    ...
    try {
      ...
      const res = await fetch('/api/foods', {
        ...
        body: JSON.stringify({
          ...
          weightPerUnit: parseFloat(weightPerUnit) || 1,
          inputUnitValue: parseFloat(weightPerUnit) || 1,
          expirationDate, expirationPrecision,
          sourceType, donorName, photoUrl,
    ...
    <div className={
      ...
      {currentView === 'FORM_VIEW' && (
        <FormView 
          ...
          weightPerUnit={weightPerUnit} setWeightPerUnit={setWeightPerUnit}
          expirationPrecision={expirationPrecision} setExpirationPrecision={setExpirationPrecision}
