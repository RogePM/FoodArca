      ...
      <div className="flex-1 overflow-y-auto px-4 py-4 space-y-5 pb-6 pt-[max(env(safe-area-inset-top,16px),16px)]">
        ...
        <section>
          <div className="bg-white rounded-[24px] border border-gray-100 shadow-[0_2px_10px_-4px_rgba(0,0,0,0.02)] flex flex-col overflow-hidden divide-y divide-gray-100/80">
            ...
            {/* AMOUNT */}
            <div className="flex items-center justify-between min-h-[64px] px-5 py-3">
              <span className="text-[16px] font-medium text-gray-900 shrink-0">Amount</span>
              <div className="flex items-center flex-1 justify-end">
                <input
                  ...
                  className="flex-1 max-w-[80px] text-right text-[17px] font-bold text-[#d97757] outline-none placeholder:text-[#d97757]/40 bg-transparent mr-3"
                />
                
                {/* Separator to prevent accidental clicks */}
                <div className="w-[1px] h-6 bg-gray-200/80 mr-3" />
                
                <div className="relative flex items-center min-w-[3.5rem]">
                  <select 
                    value={unit} 
                    onChange={(e) => setUnit(e.target.value)}
                    className="absolute inset-0 w-full h-full opacity-0 z-10 text-[16px]"
                  >
                  ...
                  <div className="flex items-center justify-end w-full text-[15px] font-medium text-gray-500 pointer-events-none">
                    {unit.charAt(0).toUpperCase() + unit.slice(1)}
                    <ChevronDown className="h-4 w-4 text-gray-400 ml-1" strokeWidth={2.5} />
                  </div>
                </div>
              </div>
            </div>
            ...
            {/* EXPIRATION */}
            <div className="flex items-center justify-between min-h-[64px] px-5 py-3">
              <span className="text-[16px] font-medium text-gray-900 shrink-0 mr-4">Expiration</span>
              <input
                ...
                className="flex-1 text-right text-[15px] font-medium text-gray-500 outline-none bg-transparent placeholder:text-gray-400"
              />
            </div>
