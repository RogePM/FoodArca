...
export function DistributionMobileList({ inventory }) {
  // 1. State Management
  const [view, setView] = useState('CART_VIEW');
  ...
  const handleAddSuggestion = (group) => {
    setClientCart(prev => {
      const existing = prev.find(item => item.mainItem._id === group.mainItem._id);
      if (existing) {
        return prev.map(item => 
          item.mainItem._id === group.mainItem._id 
            ? { ...item, quantity: Math.min(item.quantity + 1, group.totalQuantity) } 
            : item
        );
      }
      return [...prev, { mainItem: group.mainItem, batches: group.batches, totalQuantity: group.totalQuantity, quantity: 1 }];
    });
    setSearchInput('');
  ...
  const handleUpdateQuantity = (id, newQuantity) => {
    setClientCart(prev => prev.map(item => {
      if (item.mainItem._id === id) {
        const maxQty = item.totalQuantity;
        const validQty = typeof newQuantity === 'number' && !isNaN(newQuantity) ? Math.min(Math.max(1, newQuantity), maxQty) : newQuantity;
  ...
  const handleRemoveItem = (id) => {
    setClientCart(prev => prev.filter(item => item.mainItem._id !== id));
  };
  ...
  const totalItemsInCart = clientCart.reduce((sum, item) => sum + (typeof item.quantity === 'number' ? item.quantity : 0), 0);

  if (view !== 'CART_VIEW') return null;

  return (
    <div className="md:hidden flex flex-col h-full font-sans bg-[#f7f7f5] overflow-hidden">
      ...
      <div className="px-4 py-4 z-20 shrink-0">
        <div className="relative">
          <div className="relative flex items-center bg-white border border-gray-200 rounded-[16px] h-14 shadow-sm focus-within:ring-2 focus-within:ring-[#154734]/20 focus-within:border-[#154734] transition-all">
            ...
            />
            <button 
              onClick={() => console.log('Scanner coming next')}
      ...
      <div className="flex-1 min-h-0 px-4 overflow-y-auto pb-4">
        ...
        {clientCart.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-20 text-center px-4">
            ...
            <button 
              onClick={() => console.log('Scanner coming next')}
              className="h-12 px-6 bg-[#d97757]/10 active:bg-[#d97757]/20 text-[#d97757] rounded-full flex items-center justify-center gap-2 font-bold text-[15px] transition-colors"
            >
          ...
          <div className="flex flex-col gap-3">
            {clientCart.map((item) => {
              ...
              return (
                <div key={item.mainItem._id} className="bg-white rounded-[20px] p-4 shadow-[0_2px_10px_-4px_rgba(0,0,0,0.04)] border border-gray-100 flex items-center justify-between">
                  <div className="flex items-center gap-3">
                  ...
                  <div className="flex items-center gap-1.5 shrink-0">
                    <div className="flex items-center bg-gray-50 border border-gray-200/60 rounded-xl px-1 shadow-sm">
                      <input 
                        ...
                        onChange={(e) => handleUpdateQuantity(item.mainItem._id, e.target.value ? parseInt(e.target.value) : '')}
                        onBlur={(e) => {
                          if (!e.target.value || parseInt(e.target.value) < 1) handleUpdateQuantity(item.mainItem._id, 1);
                        }}
                    ...
                    <button 
                      onClick={() => handleRemoveItem(item.mainItem._id)}
      ...
      {/* 4. BOTTOM BAR: Sticky Footer */}
      <div className="shrink-0 w-full bg-white border-t border-gray-100 px-4 pt-3 pb-3 z-30">
        <div className="grid grid-cols-3 gap-3 max-w-md mx-auto">
          ...
          <button 
            onClick={() => console.log('Review coming next')}
          ...
          >
            ...
            <span className="truncate">Review ({totalItemsInCart})</span>
          </button>
        </div>
      </div>
