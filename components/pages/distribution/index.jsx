...
export function DistributionModule({ initialInventory = [] }) {
  ...
  return (
        <div className="w-full max-w-[100vw] overflow-x-hidden bg-[#f7f7f5] font-sans">
            ...
            {/* --- CART BUTTON (HIDDEN ON DESKTOP WHERE SIDEBAR IS VISIBLE) --- */}
            {cart.length > 0 && (
                <div className="lg:hidden fixed bottom-[110px] right-5 z-[90] animate-in slide-in-from-bottom-5 fade-in duration-300">
                    <button
                        onClick={() => setIsCartOpen(true)}
                        className="bg-[#154734] hover:bg-[#0f3526] text-white rounded-full h-[56px] pl-2.5 pr-5 flex items-center gap-3 shadow-[0_12px_30px_-6px_rgba(21,71,52,0.6)] active:scale-[0.95] transition-all border-2 border-white/10"
                    >
                        {/* The Count Badge (White circle with green text) */}
                        <div className="bg-white text-[#154734] rounded-full h-[38px] min-w-[38px] flex items-center justify-center font-black text-[16px] shadow-sm">
                            {cart.reduce((a, c) => a + c.quantity, 0)}
                        </div>
                        
                        {/* The Text & Icon */}
                        <div className="flex items-center gap-2 font-bold text-[15px] tracking-tight">
                            View Cart <ShoppingBag className="h-[18px] w-[18px] mb-0.5" strokeWidth={2.5} />
                        </div>
                    </button>
                </div>
            )}
