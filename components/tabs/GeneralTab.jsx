...
import {
  Loader2, Mail, CheckCircle2, Plus, Copy, MapPin, Trash2,
...
export function GeneralTab({ supabase, pantryId, details, userRole, currentPlan, refreshPantry, hasProFeatures, usageStats }) {
  ...
  const handleSendInvite = async (e) => {
    ...
    if (isSeatsFull) {
      alert(`Seat limit reached (${maxSeats}). Please upgrade.`);
      return;
    }

    setIsInviting(true);
    try {
      // 1. Get current user ID for 'invited_by'
      ...
      if (refreshPantry) await refreshPantry();
