...
with open('components/pages/distribution/mobile-checkout-cart-view.jsx', 'r', encoding='utf-8') as f:
    content = f.read()

# 1. Replace the CART HEADER and the items map with the new layout
new_cart_content = '''            {/* ── SEARCH BAR (FILLED STATE) ── */}
...
content = re.sub(r'\{\/\* .*? CART HEADER \(when items exist\).*?<\/AnimatePresence>', new_cart_content, content, flags=re.DOTALL)

# 2. Fix the extra </div> that was left behind after the AnimatePresence and Clear Cart button
# The old code had:
#             <div className="flex justify-center pt-4">
#               <button ...> Clear checkout cart </button>
#             </div>
#             </div>
# We need to remove that extra </div>
content = re.sub(r'(Clear checkout cart\s*</button>\s*</div>)\s*</div>', r'\1', content, flags=re.DOTALL)

# 3. Remove the Search FAB
new_fab_content = '''      {/* FLOATING ACTION BUTTONS (FABs) */}
      <AnimatePresence>
        {cartItems.length > 0 && (
          <motion.div
          ...
          >
            <button
              ...
              title="Scan Barcode"
            >'''
content = re.sub(r'\{\/\* FLOATING ACTION BUTTONS \(FABs\).*?title="Scan Barcode"\s*>', new_fab_content, content, flags=re.DOTALL)

with open('components/pages/distribution/mobile-checkout-cart-view.jsx', 'w', encoding='utf-8') as f:
    f.write(content)
